...
function badgeos_get_login_or_achievements(){
    ...
    foreach($achievements_all as $k => $id){
        ...
        //Remove current post achievement id, remining achievements display under login-or achievements list
 ...
 * @since  1.0.0
 */
function login_or_achievements_skip_ajax(){
    ...
    if($achievement_id){
        ...
        $message = __( 'Achievement skipped successfully', 'badgeos-login-or-achievements' );
        ...
        $message = __( 'Achievement not available', 'badgeos-login-or-achievements' );
    }
...
add_action( 'wp_ajax_login_or_achievements_skip_ajax', 'login_or_achievements_skip_ajax' );
...
add_filter('next_post_link', 'badgeos_login_or_achievements_skip_link');
function badgeos_login_or_achievements_skip_link($link) {
    global $post;
    $achievement_id = ( badgeos_is_achievement($post) )? $post->ID : "";
    $link = str_replace('" rel="next">', '?next" rel="next">', $link);

    if( class_exists( 'BadgeOS_login_or_achievements' ) &&
